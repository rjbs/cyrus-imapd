    ...
    *mailboxptr = NULL;
    ...
    if (mailbox->i.options & OPT_MAILBOX_DELETED) {
        ...
        if (!r && (mailbox->i.options & OPT_MAILBOX_DELETED)) {
            mailbox_delete_cleanup(mailbox_partition(mailbox), mailbox_name(mailbox),
...
static int mailbox_read_header(struct mailbox *mailbox, const char *fname)
{
    ...
    if (!tab || tab > eol) {
        xsyslog_ev(LOG_DEBUG, "mailbox.header.old_format",
                   lf_mailbox(mailbox));
        tab = eol;
    }
    if (p < tab) {
                      ...
                      int *flagnum, int create)
{
    ...
    if (userflag == MAX_USER_FLAGS) {
        ...
        if (create == 1 && _too_many_flags(flag, emptyflag)) {
            xsyslog_ev(LOG_ERR, "mailbox.userflag.exhausted",
                       lf_mailbox(mailbox),
                       lf_s("msg.flag", flag));
            return IMAP_USERFLAG_EXHAUSTED;
        }
                                       ...
                                       struct index_header *i)
{
    ...
    if (i->minor_version < 6 || i->minor_version > MAILBOX_MINOR_VERSION) {
        return IMAP_MAILBOX_BADFORMAT;
    }
    ...
    const index_file_template_t *template =
        index_files_by_version[i->minor_version];
    if (!template) {
        return IMAP_MAILBOX_BADFORMAT;
    }

    size_t headerlen = template->header_size;
    if (len < headerlen ||
        i->start_offset != headerlen ||
        (i->record_size != template->record_size &&
         // allow reading "bad" files with v19 header, but v20 record sizes
         (i->minor_version != 19 ||
          i->record_size != index_files_by_version[20]->record_size))) {
        return IMAP_MAILBOX_BADFORMAT;
    }

    /* Read remaining header fields */
    ptr = index_file_read_fields(ptr, i, template->header_fields);

    if (template->crc_field) {
        /* CRC is always the last 4 bytes */
        uint32_t crc;
        ptr = index_file_read_fields(ptr, &crc, template->crc_field);
...
static size_t mailbox_record_offset(const struct mailbox *m, uint32_t recno)
{
    return (size_t)m->i.start_offset + (size_t)(recno - 1) * m->i.record_size;
}
...
static int mailbox_refresh_index_map(struct mailbox *mailbox)
{
    ...
    need_size = (size_t)mailbox->i.start_offset +
                (size_t)mailbox->i.num_records * mailbox->i.record_size;
                                       ...
                                       struct index_record *record, int dirty)
{
    ...
    if (template->crc_field) {
        ...
        if (!dirty) {
            /* check CRC32 */
            if (crc != crc32_map(buf, recordlen-4))
                r = IMAP_MAILBOX_CHECKSUM;
        }
    }
...
static int _commit_one(struct mailbox *mailbox, struct index_change *change)
{
    ...
    offset = mailbox_record_offset(mailbox, recno);
...
static int _commit_changes(struct mailbox *mailbox)
{
    ...
    mailbox->index_size = (size_t)mailbox->i.start_offset +
                          (size_t)mailbox->i.num_records * mailbox->i.record_size;
...
static int mailbox_lock_index_internal(struct mailbox *mailbox, int index_locktype)
{
    ...
    if (mailbox->header_file_crc && mailbox->i.header_file_crc &&
        ...
        xsyslog_ev(LOG_WARNING, "mailbox.header.crc.mismatch",
                   lf_mailbox(mailbox),
                   lf_llx("mbox.crcs.header", mailbox->header_file_crc),
                   lf_llx("old.mbox.crcs.header", mailbox->i.header_file_crc));
    }
...
EXPORTED void mailbox_unlock_index(struct mailbox *mailbox, struct statusdata *sdata)
{
    ...
    if (r) {
        xsyslog_ev(LOG_ERR, "mailbox.commit.failed",
                   lf_mailbox(mailbox));
        abort();
    }
    ...
    if (mailbox->index_locktype) {
        if (lock_unlock(mailbox->index_fd, index_fname))
            xsyslog_ev(LOG_ERR, "mailbox.unlock.failed",
                       lf_mailbox(mailbox));
        ...
        if (timediff > 1.0) {
            xsyslog_ev(LOG_NOTICE, "mailbox.lock.slow",
                       lf_mailbox(mailbox),
                       lf_duration("mbox.lock.duration", timediff));
        }
    }
    ...
    if (mailbox->cstate_flag == CSTATE_FLAG_LOCAL) {
        ...
        if (r) {
            xsyslog_ev(LOG_ERR, "mailbox.conversations.commit.failed",
                       lf_mailbox(mailbox),
                       lf_err("error", r));
        }
    }
...
static int mailbox_commit_header(struct mailbox *mailbox)
{
    ...
    if (fd == -1) {
        xsyslog_ev(LOG_ERR, "mailbox.header.write.failed",
                   lf_s("sys.path", newfname));
        return IMAP_IOERROR;
    }
...
static bit32 mailbox_index_header_to_buf(struct index_header *i,
                                         unsigned char *buf)
{
    int version = i->minor_version;

    assert(version >= 6 && version <= MAILBOX_MINOR_VERSION);

    const index_file_template_t *base_template = index_files_by_version[0];
    const index_file_template_t *template = index_files_by_version[version];
    unsigned headerlen = template->header_size;
    unsigned char *ptr;
    bit32 crc = 0;

    memset(buf, 0, headerlen); /* buffer is always this big, and aligned */

    /* Normalize header fields */
    i->options &= template->options_mask;

    /* Write "base" header fields */
    ptr = index_file_write_fields(i, buf, base_template->header_fields);

    /* Sanity check pointer location in buffer */
    assert(base_template->header_size == (unsigned) (ptr - buf));

    /* Write version-specific header fields */
    ptr = index_file_write_fields(i, ptr, template->header_fields);

    if (template->crc_field) {
        /* Update checksum (always the last 4 bytes) */
        crc = crc32_map((char *)buf, headerlen-4);
        ptr = index_file_write_fields(&crc, ptr, template->crc_field);
    }

    /* Sanity check pointer location in buffer */
    assert(template->header_size == (unsigned) (ptr - buf));
                                         ...
                                         unsigned char *buf)
{
    ...
    copyrecord.system_flags |= copyrecord.internal_flags;

    ptr = index_file_write_fields(&copyrecord, buf, template->record_fields);

    if (template->crc_field) {
        /* calculate the checksum */
        crc = crc32_map((char *)buf, recordlen-4);
        ptr = index_file_write_fields(&crc, ptr, template->crc_field);
    }
                                          ...
                                          struct index_record *record)
{
    ...
    /* expunged tracking */
    if (record->internal_flags & FLAG_INTERNAL_EXPUNGED &&
        (!mailbox->i.first_expunged.tv_sec ||
         mailbox->i.first_expunged.tv_sec > record->last_updated.tv_sec))
        mailbox->i.first_expunged.tv_sec = record->last_updated.tv_sec;
                                ...
                                struct mailbox_repack **repackptr, int flags)
{
    ...
    if (repack->newmailbox.index_fd == -1) {
        xsyslog_ev(LOG_ERR, "mailbox.repack.setup.failed",
                   lf_s("sys.path", fname));
        goto fail;
    }
    ...
    repack->newmailbox.i.start_offset = template->header_size;
    repack->newmailbox.i.record_size = template->record_size;
    if (!(repack->newmailbox.i.start_offset && repack->newmailbox.i.record_size))
    ...
    if (version >= 12 && mailbox->i.minor_version < 12) {
        ...
        if (!r) {
            ...
            repack->newmailbox.i.recenttime.tv_sec = sd.lastchange;
...
HIDDEN int mailbox_repack_commit(struct mailbox_repack **repackptr)
{
    ...
    if (repack->newmailbox.i.minor_version > 10 &&
        ...
        xsyslog_ev(LOG_ERR, "mailbox.repack.crc.mismatch",
                   lf_mailbox(repack->mailbox),
                   lf_llx("old.mbox.crcs.basic", repack->crcs.basic),
                   lf_llx("mbox.crcs.basic", repack->newmailbox.i.synccrcs.basic),
                   lf_llx("old.mbox.crcs.annot", repack->crcs.annot),
                   lf_llx("mbox.crcs.annot", repack->newmailbox.i.synccrcs.annot));
        r = IMAP_MAILBOX_CHECKSUM;
        goto fail;
    }

    repack->newmailbox.i.last_repack_time.tv_sec = time(0);
    ...
    if (repack->mailbox->i.minor_version >= 12 && repack->newmailbox.i.minor_version < 12
        ...
        if (!r) {
            ...
            sd.lastchange = repack->newmailbox.i.last_appenddate.tv_sec;
    ...
    for (i = 0; i < cachefiles.count; i++) {
        ...
        if (xunlink(fname) == 0)
            xsyslog_ev(LOG_NOTICE, "mailbox.cache.file.removed",
                       lf_s("sys.path", fname));
    }
                                 ...
                                 ptrarray_t *records)
{
    ...
    xsyslog_ev(LOG_INFO, "mailbox.repack.started",
               lf_mailbox(mailbox),
               lf_d("mbox.version", version),
               lf_flag("mbox.repack.dryrun", dryrun),
               lf_flag("mbox.repack.recalc_nanosec", recalc_nanosec));
    ...
    while ((msg = mailbox_iter_step(iter))) {
        ...
        if (copyrecord.internal_flags & FLAG_INTERNAL_UNLINKED) {
            ...
            if (copyrecord.modseq > repack->newmailbox.i.deletedmodseq) {
                ...
                repack->newmailbox.i.changes_epoch.tv_sec = copyrecord.last_updated.tv_sec;
            }
            continue;
        }
                                     ...
                                     unsigned *ndeleted, int limit)
{
    ...
    if (dirty) {
        mailbox_index_dirty(mailbox);
        mailbox->i.options |= OPT_MAILBOX_NEEDS_REPACK;
        mailbox->i.first_expunged.tv_sec = first_expunged;
    }
                   ...
                   struct mailbox **mailboxptr)
{
    ...
    assert(minor_version >= 6 && minor_version <= MAILBOX_MINOR_VERSION);

    /* init non-zero fields */
    const index_file_template_t *template =
        index_files_by_version[minor_version];
    mailbox_index_dirty(mailbox);
    mailbox->i.minor_version = minor_version;
    mailbox->i.start_offset = template->header_size;
    mailbox->i.record_size = template->record_size;
    ...
    mailbox->i.changes_epoch.tv_sec = time(0);
    mailbox->i.synccrcs.basic = CRC_INIT_BASIC;
    mailbox->i.synccrcs.annot = CRC_INIT_ANNOT;

    /* initialise header size field so appends calculate the
     * correct map size */
    mailbox->index_size = template->header_size;
...
HIDDEN int mailbox_delete_cleanup(const char *part,
                                  const char *name,
                                  const char *uniqueid)
{
...
fail:
    ...
    mailbox_delete_cleanup(mailbox_partition(newmailbox), mailbox_name(newmailbox),
                                    ...
                                    int nolink, int *dirfdp)
{
    ...
    if (cyrus_copyfile_fdptr(from, to, flags, dirfdp))
        return IMAP_IOERROR;
                                     ...
                                     struct index_record *record)
{
    ...
    offset = mailbox_record_offset(mailbox, record->recno);
                                             ...
                                             struct index_record *record)
{
    ...
    if (cyrus_settime_fdptr(fname, &record->internaldate, fdptr) < 0) {
        xsyslog_ev(LOG_ERR, "mailbox.file.mtime.failed",
                   lf_s("sys.path", fname));
        return IMAP_IOERROR;
    }
